...
class DatabaseHelper {
  ...
  void _onCreate(Database db, int newVersion) async {
    String content = await rootBundle.loadString("assets/sql/create.sql");
    List<String> commands = content.split(";");
    commands = commands.map((sql) => sql.trim()).toList();
    for(String sql in commands){
      if(!sql.isEmpty) await db.execute(sql);
    }
  }
